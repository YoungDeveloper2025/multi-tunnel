...
def native_reverse(s):
    """Keep older layouts readable; schemas 3/4 use native reverse engines."""
    return s.get('schema_version') in (3,4) and s['engine'] in ('backhaul', 'rathole')
def smite_gost(s):
    return s.get('schema_version') == 4 and s['engine'] == 'gost'
def gost_input_transport(s):
    # Smite places this transport on the Iran listener, not on an outer carrier.
    # GOST 3 calls TCP multiplexing mtcp; tcpmux:// silently falls back to TCP.
    return 'mtcp' if s['transport']=='tcpmux' else s['transport']
...
def compile_spec(s, role, base):
    ...
    files = {}
    if smite_gost(s):
        if role=='foreign': return files
        wire = gost_input_transport(s)
        protocol = 'udp' if wire=='udp' else 'tcp'
        def forward(name,listen,target):
            service = _gost_forward(name,listen,target,protocol)
            service['listener']['type'] = wire
            if wire not in ('tcp','udp'): service['handler']['type'] = 'forward'
            return service
        services = [forward('p'+str(i),'0.0.0.0:'+str(p),_gost_addr(s['endpoint'],q)) for i,(p,q) in enumerate(s['maps'])]
        target = _gost_addr(s['endpoint'],s['health_f'])
        config = {'services':services}
        if wire in ('tcp','udp'):
            services.append(forward('health','127.0.0.1:'+str(s['health_i']),target))
        else:
            # Exercise the selected input transport as well as the Foreign HMAC
            # responder. Both adapter sockets are local to Iran, in one process.
            services.append(forward('health-wire','127.0.0.1:'+str(s['monitor']),target))
            health = _gost_forward('health','127.0.0.1:'+str(s['health_i']),target)
            health['handler']['chain'] = 'health-input'
            services.append(health)
            config['chains'] = [{'name':'health-input','hops':[{'name':'local','nodes':[{
                'name':'health-wire','addr':'127.0.0.1:'+str(s['monitor']),
                'connector':{'type':'forward'},'dialer':{'type':wire}}]}]}]
...
def validate_shared(s):
    ...
    if type(s['schema_version'])!=int or s['schema_version'] not in (2,3,4): raise ValueError('Unsupported pairing version. Update the manager on both servers.')
    valid_id(s['id'])
    ...
    if smite_gost(s) and s['cdn']: raise ValueError('Smite GOST forwards to Foreign directly and does not create a Cloudflare WSS carrier. Use a Foreign IP or DNS-only domain.')
    if s['cdn']:
    ...
    return s
def simple_gost(s): return s['engine']=='gost' and (s['transport'] in ('tcp','udp') or smite_gost(s))
...
def decode_pair(value):
    if not value.startswith(('MULTI2.','MULTI3.','MULTI4.')) or len(value)>65536: raise ValueError('Invalid connection code.')
    try:
...
def confirm_yn_default(label, default='y'):
    ...
    while True:
        ...
        print('Please enter y or n.')
def all_manifests():
    return [read(p) for p in ROOT.glob('[a-z0-9]*/manifest.json') if read(p,{}).get('schema_version') in (2,3,4)]
def legacy_ids():
    return [p.parent.name for p in ROOT.glob('[a-z0-9]*/spec.json') if read(p,{}).get('schema_version') not in (2,3,4)]
def protocols(s):
...
def claims(s):
    if s['role']=='iran':
        ...
        if smite_gost(s) and s['transport'] not in ('tcp','udp'): out += [(s['monitor'],'tcp','127.0.0.1')]
        if s['engine']!='gost':
...
def firewall_rules(s):
    ...
    if simple_gost(s): return [[q,t,'any'] for _,q in s['maps'] for t in protocols(s)]+[[s['health_f'],t,'any'] for t in ('tcp','udp')]
...
def validate_local_directory(base):
    ...
    if not isinstance(s,dict) or not isinstance(m,dict) or m.get('schema_version') not in (2,3,4): raise ValueError('Missing or incompatible local manifest: '+base.name)
...
def deploy(s,tls,require_probe=False):
    ...
    try:
        ...
        if require_probe:
            try:
                verified = probe(s,attempts=60,budget=10 if direct_gost_tcp(s) else 60)
                connection_ok = True
            except ProbeFailure as error:
                # Direct TCP application forwarding does not depend on the
                # separate Foreign health responder. Keep it installed, but
                # never turn an unverified connection into a success result.
                if not direct_gost_tcp(s): raise
                probe_error = str(error)
                connection_ok = False
            if direct_gost_tcp(s) and run(['systemctl','is-active',service(s,'gost')],check=False)!='active':
                raise RuntimeError('GOST did not stay active during the connection test.')
    except BaseException:
        recover(ident)
        raise
    ...
    if s['role']=='foreign':
        if direct_gost_tcp(s): print(gost_foreign_diagnostic(s))
        print('This side is ready. Configure Iran with the connection code; tunnel traffic is not verified yet.')
        show_code(s)
    elif probe_error:
        report_probe_failure(s,probe_error)
        print('GOST TCP is installed. Its separate health check is unverified; application traffic must be checked separately.')
    elif verified is not None:
        print('Authenticated health check successful. Application traffic is not verified.' if direct_gost_tcp(s) else 'Tunnel connection successful.')
        print(verified)
        if direct_gost_tcp(s): print(gost_tcp_diagnostic(s))
    else:
        connection_ok = run_connection_test(s)
        if not connection_ok: print('Configuration saved. Configure/check Foreign, then use test tunnel connection.')
    if s['sniffer']: print('Local sniffer: http://127.0.0.1:'+str(s['monitor']))
    return {'installed':True,'connection_ok':connection_ok,'probe_error':probe_error}
...
def recv_exact(conn,n,deadline=None):
    data = b''
    while len(data)<n:
...
def echo_service(s):
    ...
    tcp.bind((address,s['health_f'])); tcp.listen(64); udp.bind((address,s['health_f']))
    ...
    def serve_client(conn):
        try:
            with conn:
                conn.settimeout(2)
                try:
                    answer = health_reply(s,recv_exact(conn,64,time.monotonic()+2))
    ...
    def serve_tcp():
        while True:
            ...
            if not slots.acquire(blocking=False):
                conn.close(); continue
            try: threading.Thread(target=serve_client,args=(conn,),daemon=True).start()
            except RuntimeError:
...
class ProbeFailure(RuntimeError):
    """An authenticated health exchange failed, rather than installation."""

def direct_gost_tcp(s):
    return s.get('engine')=='gost' and s.get('transport')=='tcp'
...
def probe(s,attempts=1,budget=20):
    ...
    for attempt in range(attempts):
        try:
            for proto in protocols(s):
                ...
                if simple_gost(s): target += ' -> '+_gost_addr(s['endpoint'],s['health_f'])+' (Foreign health responder)'
                ...
                with socket.socket(socket.AF_INET,socket.SOCK_DGRAM if proto=='udp' else socket.SOCK_STREAM) as sock:
                    ...
                    sock.connect(('127.0.0.1',s['health_i']))
                    remaining = attempt_deadline-time.monotonic()
                    if remaining<=0: raise TimeoutError('Health-test deadline reached')
                    sock.settimeout(remaining); sock.sendall(request)
                    stage = 'authenticated end-to-end reply'
                    remaining = attempt_deadline-time.monotonic()
                    if remaining<=0: raise TimeoutError('Health-test deadline reached')
                    sock.settimeout(remaining)
                    reply = sock.recv(1024) if proto=='udp' else recv_exact(sock,64,attempt_deadline)
                expected = request[:32]+hmac.digest(health_key(s),b'response'+request[:32],'sha256')
                if len(reply)!=64: raise RuntimeError('Incomplete authenticated reply ('+str(len(reply))+'/64 bytes). Check the Foreign health responder and matching connection code.')
                if not hmac.compare_digest(reply,expected): raise RuntimeError('Authenticated response mismatch. Check that both servers use the same connection code.')
            ...
            # Storage/programming errors must not be mistaken for an optional
            # network probe failure by the deployment transaction.
    ...
    raise ProbeFailure(last or 'No response from the other server.')
def gost_tcp_diagnostic(s,budget=8):
    """Check actual Iran listeners and Foreign TCP reachability separately."""
    lines = ['Separate health endpoint on Foreign: '+_gost_addr(s['endpoint'],s['health_f'])+'/tcp.',
             'Check multi-'+s.get('peer_id',s['id'])+'-echo.service on Foreign and this health port in the provider/host firewall.',
             'If Foreign was edited/replaced, import its current connection code on Iran; a stale code fails authentication.',
             'Application port checks (TCP handshakes only; application traffic is not verified):']
    deadline = time.monotonic()+budget
    results = {}
    def connect(host,port,timeout):
        remaining = deadline-time.monotonic()
        if remaining<=0: return 'NOT CHECKED (time budget exhausted)'
        limit = min(timeout,remaining)
        result = []; done = threading.Event()
        def check():
            try:
                with socket.create_connection((host,port),timeout=limit): pass
                result.append('REACHABLE')
            except OSError as error: result.append('FAILED: '+str(error))
            finally: done.set()
        # getaddrinfo has no socket timeout. Keep DNS and multi-address retries
        # inside this check's wall-clock budget too.
        threading.Thread(target=check,daemon=True).start()
        if not done.wait(limit): return 'FAILED: TCP/DNS check timed out'
        return result[0] if result else 'FAILED: TCP check could not complete'
    for public,target in s['maps']:
        local = connect('127.0.0.1',public,0.5)
        if target not in results:
            results[target] = connect(s['endpoint'],target,2)
        lines.append('Iran port '+str(public)+': '+local+' | Foreign '+_gost_addr(s['endpoint'],target)+': '+results[target])
    lines.append('An accepted Iran TCP connection alone does not prove that GOST reached Foreign.')
    lines.append('Foreign applications must listen on their public interface. Test your actual client through the Iran listening ports.')
    return '\n'.join(lines)
def gost_foreign_diagnostic(s):
    """Inspect local destination bindings without changing application settings."""
    lines = ['Foreign application listeners (local bindings only; reachability from Iran is not verified):']
    try: output = run(['ss','-H','-lnt'],timeout=10)
    except (OSError,RuntimeError,subprocess.TimeoutExpired) as error: return lines[0]+'\nCould not inspect listeners: '+str(error)
    listeners = {}
    for line in output.splitlines():
        fields = line.split()
        if len(fields)<4: continue
        address,separator,number = fields[3].rpartition(':')
        if separator and number.isdigit(): listeners.setdefault(int(number),[]).append(address.strip('[]'))
    for target in sorted({q for _,q in s['maps']}):
        addresses = listeners.get(target,[])
        if not addresses: result = 'NOT LISTENING. Start the destination application on this port.'
        else:
            def loopback(address):
                try: return ipaddress.ip_address(address.split('%',1)[0]).is_loopback
                except ValueError: return False
            if all(loopback(address) for address in addresses):
                result = 'LOOPBACK ONLY ('+', '.join(addresses)+'). Direct GOST cannot reach it via the Foreign IP.'
            else: result = 'LISTENING on '+', '.join(addresses)+'; check the destination IP and host/provider firewalls.'
        lines.append('Foreign port '+str(target)+': '+result)
    return '\n'.join(lines)
def transport_diagnostic(s):
    if direct_gost_tcp(s) and s['role']=='iran': return gost_tcp_diagnostic(s)
...
def report_probe_failure(s,error):
    if direct_gost_tcp(s):
        print('Authenticated health check unsuccessful. GOST TCP application forwarding is not verified by this result.')
    else: print('Tunnel connection unsuccessful.')
...
def run_connection_test(s):
    if s['role']!='iran':
        ...
        return None
    if direct_gost_tcp(s):
        print('Testing the separate authenticated GOST TCP health path (up to 30 seconds)...',flush=True)
        print('Iran 127.0.0.1:'+str(s['health_i'])+' -> Foreign '+_gost_addr(s['endpoint'],s['health_f']))
    else: print('Testing the authenticated tunnel path (up to 30 seconds)...',flush=True)
    try: result = probe(s,attempts=30,budget=30)
    except Exception as e:
        report_probe_failure(s,e)
        return False
    print('Authenticated health check successful. Application traffic is not verified.' if direct_gost_tcp(s) else 'Tunnel connection successful.')
    print(result)
    if direct_gost_tcp(s): print(gost_tcp_diagnostic(s))
...
def ask_smite_foreign_endpoint(default=''):
    while True:
        try: endpoint = host(ask('Enter your Foreign domain or IP',default))
        except ValueError as error:
            print(str(error)); continue
        try: ipaddress.ip_address(endpoint)
        except ValueError:
            if yes('Is this domain behind Cloudflare orange proxy',True):
                print('Smite GOST forwards directly to Foreign. It does not create the old WSS carrier to Cloudflare.')
                print('Enter a directly reachable Foreign IP or a DNS-only domain instead.')
                try: default = str(ipaddress.IPv4Address(default))
                except ValueError: default = ''
                continue
        return endpoint
def describe_smite_tunnel(s):
    if native_reverse(s):
        print('Smite '+s['engine'].upper()+': server on Iran, client on Foreign; Foreign connects to Iran.')
        print('Required engine: '+s['engine'].upper()+'. No GOST carrier or automatic TLS layer is added.')
    else:
        print('Smite GOST: Iran forwards directly to Foreign service ports; no GOST receiver is installed on Foreign.')
        if s['transport'] not in ('tcp','udp'):
            print('The '+s['transport'].upper()+' input listener is on Iran. Clients must use its matching GOST transport; plain TCP clients cannot use this listener.')
def build_foreign(old=None):
    if old:
        s = dict(old)
    else:
        ident,old = _automatic_foreign_target(valid_id(ask('Tunnel name','mytunnel')))
        s = dict(old or {},id=ident)
    s.update(schema_version=4,role='foreign',revision=secrets.token_hex(16),token=secrets.token_hex(32),sniffer=False)
    ...
    if old and old['schema_version']!=4:
        print('This edit adopts the Smite connection layout. Apply the new connection code on Iran as well.')
    if native_reverse(s):
        s['endpoint'] = host(ask('Iran public IPv4 or direct DNS name',endpoint_default))
    else:
        if not endpoint_default:
        ...
        s['endpoint'] = ask_smite_foreign_endpoint(endpoint_default)
    s['cdn'] = False
    s['transport'],s['endpoint'],s['cdn'] = choose_transport(s['engine'],s['endpoint'],False,previous,endpoint_default)
    describe_smite_tunnel(s)
    ...
    s['front'] = port(ask('Iran control port',front_default)) if native_reverse(s) else front_default
...
def build_iran(old=None):
    print('First create this tunnel on Foreign, then paste its MULTI2, MULTI3 or MULTI4 connection code here.')
...
def ask_automatic_settings():
    ...
    if engine=='gost':
        ...
        endpoint_default = detect_public_ipv4()
        if not endpoint_default: print('Public IPv4 could not be detected. Enter your Foreign domain or IP manually.')
        endpoint = ask_smite_foreign_endpoint(endpoint_default)
    else:
        endpoint = iran_ip
    transport,endpoint,cdn = choose_transport(engine,endpoint,False)
    describe_smite_tunnel(dict(schema_version=4,engine=engine,transport=transport))
...
def _automatic_tunnel_id(stem='mytunnel'):
    valid_id(stem)
    for number in range(1,10001):
        suffix = '-'+str(number)
        ident = stem if number==1 else stem[:24-len(suffix)]+suffix
    ...
    raise RuntimeError('No unused tunnel name is available. Manage existing tunnels first.')
def _automatic_foreign_target(ident='mytunnel'):
    # Share the same replacement decision in manual and automatic creation.
    valid_id(ident)
    base = ROOT/ident
    remnants = [pending_path(ident),*(ROOT/(prefix+ident) for prefix in ('.stage-','.backup-','.discard-')),base/'.uncommitted',base/'.needs-restart']
    if any(path.exists() or path.is_symlink() for path in remnants): raise RuntimeError('Recover the interrupted '+ident+' operation on Foreign before creating another tunnel.')
    ...
    if old['id']!=ident or old['role']!='foreign': raise ValueError('The existing '+ident+' is not a Foreign tunnel. Use manual mode with a new name.')
    ...
    if confirm_yn_default('Foreign already has a tunnel named '+ident+'. Delete it and create the new tunnel?',default='y'):
        return ident,old
    new_id = _automatic_tunnel_id(ident)
    print('Keeping '+ident+'. New Foreign tunnel name: '+new_id)
    return new_id,None
...
def build_foreign_automatic(settings):
    ...
    if settings['cdn']: raise ValueError('Smite tunnel creation requires a direct endpoint; the old Foreign Cloudflare carrier is not used.')
    ...
    if native_reverse(s): print('Iran control port (checked on Iran before setup): '+str(s['front']))
    describe_smite_tunnel(s)
...
def status(s,logs=False):
    ...
    if native_reverse(s):
        ...
        if smite_gost(s): print('Iran input listener:',s['transport'].upper(), '| No outer GOST carrier.')
    else:
        print('Connection: Iran -> Foreign | Foreign transport endpoint:',_gost_addr(s['endpoint'],s['front']))
        if s['engine']!='gost': print('GOST TLS/WSS carrier | Foreign local bridge:',s['bridge'])
    map_label = 'Iran user port -> Foreign service port:'
    if direct_gost_tcp(s) and s['role']=='foreign':
        receipt = read(ROOT/s['id']/'iran-setup.json',{})
        current = isinstance(receipt,dict) and receipt.get('revision')==s['revision'] and receipt.get('maps')==s['maps']
        if current and receipt.get('state')=='confirmed':
            map_label = 'Iran ports confirmed at last SSH setup (application traffic not verified):'
            print('Iran tunnel name:',receipt.get('id',s['id']))
        elif current and receipt.get('state')=='pending':
            map_label = 'Planned Iran ports (remote installation not confirmed; retry SSH setup):'
        else: map_label = 'Configured Iran ports (check current ports on Iran):'
    print(map_label,', '.join(str(p)+' -> '+str(q) for p,q in s['maps']))
    if simple_gost(s): print('Separate Foreign health endpoint:',_gost_addr(s['endpoint'],s['health_f']))
    if direct_gost_tcp(s) and s['role']=='foreign': print(gost_foreign_diagnostic(s))
...
class SSHSession:
    ...
    def _redact(self, output):
        ...
        return re.sub(r'MULTI[234]\.[A-Za-z0-9_+/=-]+', '[REDACTED CONNECTION CODE]', output)
...
def validate_remote_maps(shared, maps):
    """Accept remapped Iran ports only; destination order must stay unchanged."""
    if not isinstance(maps,list) or len(maps)!=len(shared['maps']): raise ValueError('Invalid Iran port mapping result.')
    validated = []
    for pair,(_,target) in zip(maps,shared['maps']):
        if not isinstance(pair,list) or len(pair)!=2: raise ValueError('Invalid Iran port mapping result.')
        public = integer(pair[0],1,65535,'Iran port')
        if integer(pair[1],1,65535,'Foreign port')!=target: raise ValueError('Iran mapping changed a Foreign destination port.')
        validated.append([public,target])
    if len({public for public,_ in validated})!=len(validated): raise ValueError('Duplicate planned Iran ports.')
    return validated
def remote_planned_public_ports(shared, values, old=None):
    if not direct_gost_tcp(shared): raise ValueError('Explicit planned ports require GOST TCP.')
    if not isinstance(values,list) or len(values)!=len(shared['maps']): raise ValueError('Invalid planned Iran ports.')
    ports = [integer(value,1,65535,'Iran port') for value in values]
    if ports!=[public for public,_ in shared['maps']]: raise ValueError('Planned Iran ports do not match the connection code. Retry SSH setup.')
    if len(set(ports))!=len(ports): raise ValueError('Duplicate planned Iran ports.')
    other = {(p,t) for manifest in all_manifests() if not old or manifest['id']!=old['id'] for p,t,_ in manifest['claims']}
    wanted = [(p,t,'0.0.0.0') for p in ports for t in protocols(shared)]
    if any((p,t) in other for p,t,_ in wanted): raise RuntimeError('A planned Iran port was reserved by another tunnel. Retry SSH setup.')
    try: available(wanted,claims(old) if old else [])
    except RuntimeError as error: raise RuntimeError('A planned Iran port became busy. Retry SSH setup to select and save new ports. '+str(error)) from error
    return ports
...
def remote_apply(request, state):
    required = {'action','code','mode','expected_revision','binary_dir'}
    if set(request) not in (required,required|{'public_ports'}): raise ValueError('Invalid remote apply fields.')
    ...
    public_ports = remote_planned_public_ports(shared,request['public_ports'],old) if 'public_ports' in request else remote_public_ports(shared,old)
    ...
    result = deploy(s,tls,require_probe=True)
    return {'ok':True,'id':s['id'],'maps':s['maps'],**result}
def remote_dispatch(request):
    ...
    with (ROOT/'.local-v2.lock').open('w') as lock:
        ...
        if request['action']=='plan':
            ...
            if not isinstance(request['code'],str): raise ValueError('Invalid planned connection code.')
            ...
            if request['mode']=='create' and ((ROOT/shared['id']).exists() or existing): raise RuntimeError('Iran tunnel name appeared after inspection. Retry remote setup.')
            if request['mode'] in ('replace','append') and not existing: raise RuntimeError('The inspected Iran tunnel no longer exists. Retry remote setup.')
            ...
            if direct_gost_tcp(shared):
                ports = remote_public_ports(shared,old)
                return {'ok':True,'front':shared['front'],'maps':[[p,q] for p,(_,q) in zip(ports,shared['maps'])]}
...
def _remote_main():
    ...
    try:
        ...
        message = re.sub(r'MULTI[234]\.[A-Za-z0-9_=-]+','[redacted]',message)
...
def configure_iran_session(s, session, *, via_foreign=False, automatic=False, dependencies_ready=False):
    ...
    try:
        ...
        if direct_gost_tcp(s):
            ...
            maps = validate_remote_maps(s,planned.get('maps'))
            if maps!=s['maps']:
                print('Saving the actual Iran listening ports on Foreign before creating the Iran tunnel.')
                s = dict(s,maps=maps)
                deploy(s,{})
                code = foreign_connection_code(s)
            planned_ports = [public for public,_ in maps]
            write(ROOT/s['id']/'iran-setup.json',json.dumps({'revision':s['revision'],'maps':s['maps'],'state':'pending'}))
        elif native_reverse(s):
            planned = remote_request(session,remote_script,{'action':'plan','code':code,'mode':mode,'expected_revision':state.get('existing_revision')})
        ...
        apply_request = {'action':'apply','code':code,'mode':mode,'expected_revision':state.get('existing_revision'),'binary_dir':stage+'/bin'}
        if planned_ports is not None: apply_request['public_ports'] = planned_ports
        result = remote_request(session,remote_script,apply_request,timeout=1200)
        if planned_ports is not None and validate_remote_maps(s,result.get('maps'))!=s['maps']: raise RuntimeError('Iran returned unexpected listening ports. Inspect the tunnel before using it.')
        unverified_tcp = direct_gost_tcp(s) and result.get('ok') is True and result.get('installed') is True and result.get('connection_ok') is False
        if result.get('connection_ok') is not True and not unverified_tcp: raise RuntimeError('Iran has not verified a successful tunnel connection.')
        if planned_ports is not None:
            write(ROOT/s['id']/'iran-setup.json',json.dumps({'revision':s['revision'],'maps':s['maps'],'state':'confirmed','id':valid_id(result['id']),'connection_ok':result['connection_ok']}))
        ...
        if unverified_tcp:
            print('Iran GOST TCP installation completed; the separate authenticated health check is unverified.')
            print('Health check reason:',result.get('probe_error') or 'No authenticated response.')
            print('Check the Foreign health endpoint '+_gost_addr(s['endpoint'],s['health_f'])+' and test your application through the Iran ports above.\n')
        elif direct_gost_tcp(s): print('Iran ports synchronized. Authenticated health check passed; test application traffic with your client.\n')
        else: print('Tunnel connection successful. Application services must be checked separately.\n')
        return result
    finally:
        with ignore_interrupts():
...
def create_automatic_foreign():
    ...
    try:
        ...
        with SSHSession(settings['iran_ip'],ssh_port,password) as session:
            ...
            unverified_tcp = result and direct_gost_tcp(s) and result.get('ok') is True and result.get('installed') is True and result.get('connection_ok') is False
            if not result or (result.get('connection_ok') is not True and not unverified_tcp): raise RuntimeError('Iran did not confirm the tunnel connection.')
            if unverified_tcp: print('Automatic installation completed on both servers; GOST TCP connectivity remains unverified.')
            elif direct_gost_tcp(s): print('Automatic setup completed on both servers. Health check passed; application traffic must be tested separately.')
            else: print('Automatic setup completed on both servers.')
            return result
...
def main():
    ...
    if len(sys.argv)>1 and sys.argv[1] in ('echo','guard-on','guard-off'):
        s = read(sys.argv[2])
        if s.get('schema_version') not in (2,3,4): raise ValueError('Unsupported local configuration.')
