...
def ensure_dependencies():
    ...
    run_interactive(["apt-get", "install", "-y", "--no-install-recommends", *packages], timeout=600, env=env)
...
def choose(label, options, default):
    ...
    labels.update({'panel3x':'نصب آخرین نسخه پنل mhsanaei/3x-ui', 'panelx':'نصب آخرین نسخه پنل alireza0/x-ui'})
...
def get_cert(default_domain='', *, interactive=True):
    ...
    try:
        if interactive: run_interactive(['apt-get','install','certbot','-y'],check=True,timeout=900)
        else:
            ...
            run_interactive(['apt-get','-o','Dpkg::Options::=--force-confdef','-o','Dpkg::Options::=--force-confold','install','certbot','-y'],check=True,timeout=900,env=env)
...
def ask_automatic_settings():
    ...
    while True:
        try:
            endpoint = host(ask('enter your foriegn domain or ip',endpoint_default))
            break
        except ValueError as error: print(str(error))
    try: ipaddress.ip_address(endpoint)
    except ValueError: cdn = yes('Is this domain behind Cloudflare orange proxy',False)
    else: cdn = False
    ...
    transport_default = 'ws' if cdn and engine=='gost' else 'tcp'
    ...
    while True:
        try:
            ...
            if cdn and engine=='gost' and transport!='ws':
                if endpoint_default: print('Direct connection target: '+endpoint_default)
...
def build_foreign_automatic(settings):
    ...
    if not simple_gost(s): print('Foreign transport port: '+str(s['front'])+(' (8443 was unavailable)' if s['front']!=8443 else ''))
    if s['cdn']: print('WebSocket over TLS was selected automatically for the outer Cloudflare connection.')
    if s['engine']!='gost': print('The Foreign endpoint uses a GOST TLS/WSS carrier.')
...
def manage():
    ...
    choices = ('status','test','edit','delete','restart','logs','code','cert','ssh','panel3x','panelx') if s['role']=='foreign' else ('status','test','edit','delete','restart','logs','cert','panel3x','panelx')
    ...
    elif action in ('panel3x','panelx'): install_panel('3x-ui' if action=='panel3x' else 'x-ui',role=s['role'])
...
def panel_dependencies():
    ...
    run_interactive(['apt-get', 'install', '-y', '--no-install-recommends', *missing], timeout=900, env=env)
