...
def gost_tcp_diagnostic(s,budget=8):
    """TCP handshakes to application targets; not an application/relay test."""
    lines = ['Separate health endpoint on Foreign: '+_gost_addr(s['endpoint'],s['health_f'])+'/tcp.',
             ...
             'Foreign application TCP reachability from Iran (does not verify GOST forwarding or application authentication):']
    deadline = time.monotonic()+budget
    results = {}
    for public,target in s['maps']:
        if target not in results:
            remaining = deadline-time.monotonic()
            if remaining<=0: results[target] = 'NOT CHECKED (diagnostic time budget exhausted)'
            else:
                try:
                    with socket.create_connection((s['endpoint'],target),timeout=min(2,remaining)): pass
                    results[target] = 'REACHABLE (TCP handshake only)'
                except OSError as error: results[target] = 'FAILED: '+str(error)
        lines.append('Iran '+str(public)+' -> '+_gost_addr(s['endpoint'],target)+': '+results[target])
