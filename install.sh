...
usage() {
    cat <<'HELP'
Install the Multi Tunnel 3.1.0 manager on Ubuntu 22.04+ (amd64).
Usage:
...
New Backhaul/Rathole tunnels use only their native engine (Foreign -> Iran).
GOST tunnels connect Iran -> Foreign. Existing schema-2 tunnels are supported.
The SSH setup verifies cached engine SHA256 values and sends only missing files.
Cloudflare questions for Foreign domains default to Yes; IPs skip that question.
...
for config in pathlib.Path('/etc/multi-tunnel').glob('*/spec.json'):
    ...
    try:
        ...
        current = isinstance(value, dict) and type(value.get('schema_version')) is int and value.get('schema_version') in (2, 3)
