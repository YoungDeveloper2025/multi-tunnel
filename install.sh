...
usage() {
    cat <<'HELP'
Install the Multi Tunnel manager on Ubuntu 22.04+ (amd64).
Usage:
...
for config in pathlib.Path('/etc/multi-tunnel').glob('*/spec.json'):
    ...
    try:
        ...
        current = isinstance(value, dict) and value.get('schema_version') == 2
...
if [[ -n "$repo" ]]; then
    ...
    curl --fail --silent --show-error --location --retry 3 --connect-timeout 20 --max-time 180 --proto '=https' --proto-redir '=https' --output "$source_file" "$source_url"
else
