...
Version **3.2.2** supports automatic setup of both servers over SSH, manual connection-code import, verified engine caching and complete local uninstallation. Menus are in English.
...
Replace both release files and rerun the local installer to update the installed manager. Schema **2**, **3** and **4** configurations remain supported. If the old 3.2.0 installer reported that a schema-4 tunnel was legacy, use **both updated 3.2.2 files** and rerun it; do not delete that tunnel to get past the old guard. Updating the manager alone leaves existing tunnels running with their saved layout.
...
Verification of 3.2.2 completed with **283 automated checks**. Fifteen checks specifically cover manual duplicate names: default/explicit confirmation, keeping the old tunnel with a free suffix, 24-character names, cancellation, damaged or interrupted configurations, wrong roles, symbolic links, changing engines, cleanup of old services, keeping cached cores and restoration after a simulated startup failure. System commands in these deployment tests are simulated; files and the builder/deployment/recovery code run in isolated temporary directories.
...
The previous 3.2.1 review passed 16 real-binary scenarios: all 13 new engine/transport combinations (including Backhaul UDP over TCP), two native wrong-token rejection cases and an existing schema-2 WSS carrier. Those checks exercised pairing, HMAC health, concurrent application port maps and native reconnects. The engine configuration generators and pinned binaries are unchanged in 3.2.2; that real-binary matrix was not rerun for this naming-menu change.
